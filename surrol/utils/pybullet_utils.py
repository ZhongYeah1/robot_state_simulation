...
def world_to_camera_position(position, view_matrix):
    ...
    pos_camera = np.dot(view_mat, pos_world)
    
    return pos_camera[:3]  # 返回相机坐标系中的3D点

def world_orientation_to_camera(orientation_euler, view_matrix):
    """将世界坐标系中的欧拉角方向转换为相机坐标系"""
    # 从欧拉角获取旋转矩阵
    ...
    # 提取视图矩阵的旋转部分
    view_rotation = np.array(view_matrix).reshape(4, 4)[:3, :3]
...
def robot_state_to_camera(robot_state, view_matrix):
    """将完整的7维robot_state转换到相机坐标系
    
    Args:
        robot_state: 7维向量 [x,y,z, roll,pitch,yaw, jaw_angle]
        view_matrix: 相机视图矩阵
        
    Returns:
        7维向量，相机坐标系下的robot_state
    """
    # 分解robot_state
    position = robot_state[:3]  # 位置：前3维
    euler_angles = robot_state[3:6]  # 欧拉角：中间3维
    jaw_angle = robot_state[6:]  # 夹持器角度：最后1维
    
    # 转换位置坐标
    camera_position = world_to_camera_position(position, view_matrix)
    
    # 转换方向(欧拉角)
    camera_euler = world_orientation_to_camera(euler_angles, view_matrix)
