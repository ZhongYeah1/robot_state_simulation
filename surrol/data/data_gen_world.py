        ...
        if args.video and current_images is not None:
            ...
            if robot_states is not None and isinstance(obs, dict) and 'observation' in obs:
                if len(obs['observation']) >= 7:
                    # 1. 处理机器人状态 (前7维) - 保持世界坐标系
                    world_state = obs['observation'][:7]
                    
                    # 2. 获取物体的位置和朝向 (保持世界坐标系)
                    needle_id = env.needle_id
                    pos, ori = p.getBasePositionAndOrientation(needle_id)
                    object_world_pos = np.array(pos)
                    object_world_ori = np.array(p.getEulerFromQuaternion(ori))
                    ...
                    full_state = (world_state.tolist() + 
                                 task_encoding + 
                                 object_world_pos.tolist() + 
                                 object_world_ori.tolist() + 
                                 goal_world_pos.tolist())
                    
                    # 打印世界坐标系数据信息
                    print(f"保存第 {len(robot_states)+1} 个状态（世界坐标系）:")
                    print(f"  机器人状态: {world_state}")
                    print(f"  物体世界位置: {object_world_pos}")
                    print(f"  物体世界朝向: {object_world_ori}")
                    print(f"  目标世界位置: {goal_world_pos}")
                    print(f"  完整19维向量长度: {len(full_state)}")
